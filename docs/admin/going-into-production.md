...
- **Local or low-latency block-backed SSDs** for any node holding actively
  written shards. CrateDB's write path generates frequent small and
  latency-sensitive I/O operations, including segment flushes, translog 
  fsyncs, and segment merges. Spinning disks and high-latency network storage
  handle such workloads poorly.
...
- **HDDs / spinning disks.** Unlike the items below, this is a pure
  ...
  hot/warm/cold storage tiering concept of its own, but allows through
  configuration to put less frequently written tables, or a node holding only
