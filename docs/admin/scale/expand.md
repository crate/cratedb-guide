...
* Optionally set a `node.name`, otherwise the node will assign a random name from the `sys.summits` table. You may wonder what those default names are about: They are the names of mountains in the area around our main office, we love mountains at [CrateDB](https://cratedb.com/).
