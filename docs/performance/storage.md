...
On reads, the index will be leveraged: Depending on the query, the engine will use the most
efficient store.
...
Data was increased `~10%` in v5.10
...
at later stages is tricky, as table will need to be re-created, which might need some downtime,
